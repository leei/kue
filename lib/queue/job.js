 ...
 * @param {String} key
 * @param {String} val
 * @param {Multi} multi (optional) A Redis multi for transactions.
 * @param {Function} fn (optional)
 * @return {Job} for chaining
 * @api public
 */

Job.prototype.set = function(key, val, multi, fn){
  if (typeof multi == 'function') {
    fn = multi; multi = null;
  }
  multi = multi || this.client;
  multi.hset('q:job:' + this.id, key, val, fn || noop);
  return this;
};
...
Job.prototype.remove = function(fn){
  ...
  this.removeState(function (err) {
    if (err) return fn(err);
    getSearch().remove(job.id);
    client.del('q:job:' + job.id + ':log');
    client.del('q:job:' + job.id, fn || noop);
  });
  return this;
};
...
Job.prototype.state = function(state, fn){
  ...
  this.removeState(function (err) {
    if (err) return fn(err);
    job._state = state;
    // Group these updates so that we know the job is properly set before we trigger Worker.
    var multi = client.multi();
    job.set('state', state, multi);
    multi.zadd('q:jobs', job._priority, job.id);
    multi.zadd('q:jobs:' + state, job._priority, job.id);
    multi.zadd('q:jobs:' + job.type + ':' + state, job._priority, job.id);
    job.set('updated_at', Date.now(), multi);
    multi.exec(function (err) {
      // increase available jobs, used by Worker#getJob()
      if (! err && 'inactive' == state)
        client.lpush('q:' + job.type + ':jobs', 1);
      fn(err);
    });
  });
  return this;
...
Job.prototype.save = function(fn){
  ...
  client.incr('q:ids', function(err, id){
    ...
    var multi = client.multi();
    if (max) multi.hset(key, 'max_attempts', max);
    multi.sadd('q:job:types', self.type);
    self.set('type', self.type, multi);
    self.set('created_at', Date.now(), multi);
    multi.exec(function (err) {
      if (err) return fn(err);
...
Job.prototype.update = function(fn){
  ...
  // delay
  if (this._delay) this.set('delay', this._delay, multi);

  // updated timestamp
  this.set('updated_at', Date.now(), multi);

  // priority
  this.set('priority', this._priority, multi);

  // data
  this.set('data', json, multi);
  ...
  multi.exec(function (err) {
    ...
    // search data
    getSearch().index(json, job.id);

    // state
    job.state(job._state, fn);
  });
};
